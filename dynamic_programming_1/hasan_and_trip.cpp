...
#include <bits/stdc++.h>
using namespace std;
long double x[100001],y[100001],f[100001];
double dist1(int a,int b)
{
    return sqrt(((x[b] - x[a]) * (x[b] - x[a])) + ((y[b] - y[a]) * (y[b] - y[a])));
}

long double fun(int n,long double *x,long double *y,long double *f)
{
    long double dist,ans=0,temp = 0;
    double dp[n];
    for(int i=0;i<n;i++)
        dp[i] = -1e9;
    dp[0] = 0;
